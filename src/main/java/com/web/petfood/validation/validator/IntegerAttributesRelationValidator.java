package com.web.petfood.validation.validator;

import com.web.petfood.validation.IntegerAttributesRelation;
import com.web.petfood.validation.util.AttributesRelation;
import com.web.petfood.validation.util.ReflectUtil;
		...
		try {
			if (field1.getType() == Integer.class) {
				...
				if (aux == null) {
					...
					ctx.buildConstraintViolationWithTemplate("O atributo " + attribute1 + " não pode ser null").addPropertyNode(attribute1).addConstraintViolation();
					return false;
				}
				value1 = aux;
			} else {
			...
			if (field2.getType() == Integer.class) {
				...
				if (aux == null) {
					ctx.disableDefaultConstraintViolation();
					ctx.buildConstraintViolationWithTemplate("O atributo " + attribute2 + " não pode ser null").addPropertyNode(attribute1).addConstraintViolation();
					return false;
				}
				value2 = aux;
			} else {
